...
HRESULT Sprite::Initialize()
{
	...
	//テクスチャのロード
	if (filename_ != "") {
		if (FAILED(LoadTextureS(filename_))) {
			return E_FAIL;
		}
	}
	else {
...
void Sprite::Draw(Transform& transform, RECT rect, float alpha)
{
	...
	//Direct3D::pContext->SetDepthBufferWriteEnable(false);
	...
	//XMMATRIX view = XMMatrixScaling(1.0f / スクリーン幅, 1.0f / スクリーン高さ, 1);
}
...
HRESULT Sprite::LoadTexture()
{
	...
	HRESULT hr;
	hr = pTexture_->Load("Assets\\NN.png");
	if (FAILED(hr))
	{
...
HRESULT Sprite::LoadTextureS(std::string filename)
{
	pTexture_ = new Texture;
	HRESULT hr;
