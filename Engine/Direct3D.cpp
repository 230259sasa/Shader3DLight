...
HRESULT Direct3D::InitShader3D()
{
	...
	hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_3D].pVertexLayout);
	...
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, 
		&shaderBundle[SHADER_TYPE::SHADER_3D].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
...
HRESULT Direct3D::InitShader2D()
{
	...
	hr = pDevice->CreateInputLayout(layout, 2, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_2D].pVertexLayout);
	...
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL, 
		&shaderBundle[SHADER_TYPE::SHADER_2D].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗", NULL, MB_OK);
...
HRESULT Direct3D::InitPointLightShader()
{
	HRESULT hr;
	...
	// 頂点シェーダの作成（コンパイル）
	ID3DBlob* pCompileVS = nullptr;
	D3DCompileFromFile(L"PointLight.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_POINT].pVertexShader);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"頂点シェーダの作成に失敗", NULL, MB_OK);
		return hr;
	...
	hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_POINT].pVertexLayout);
	...
	// ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	D3DCompileFromFile(L"PointLight.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompileVS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_POINT].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗", NULL, MB_OK);
	...
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
	hr = pDevice->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TYPE::SHADER_POINT].pRasterizerState);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ラスタライザステートの作成に失敗", NULL, MB_OK);
		return hr;
...
HRESULT Direct3D::InitShaderToon()
{
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_TOON].pVertexShader);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"頂点シェーダの作成に失敗", NULL, MB_OK);
		return hr;
	...
	hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_TOON].pVertexLayout);
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_TOON].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗", NULL, MB_OK);
	...
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
	hr = pDevice->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TYPE::SHADER_TOON].pRasterizerState);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ラスタライザステートの作成に失敗", NULL, MB_OK);
		return hr;
...
HRESULT Direct3D::InitShaderOutLine()
{
	...
	hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_OUTLINE].pVertexLayout);
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_OUTLINE].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗", NULL, MB_OK);
	...
	//ラスタライザ作成
	D3D11_RASTERIZER_DESC rdc = {};
	rdc.CullMode = D3D11_CULL_FRONT;  //多角形の裏側は描画しない（カリング）
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
...
HRESULT Direct3D::InitNormalMap()
{
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_NORMALMAP].pVertexShader);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"頂点シェーダの作成に失敗", NULL, MB_OK);
		return hr;
	...
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
		...
		{ "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT, 0, sizeof(DirectX::XMVECTOR) , D3D11_INPUT_PER_VERTEX_DATA, 0 },//UV座標
		{ "NORMAL",		0, DXGI_FORMAT_R32G32B32_FLOAT, 0, sizeof(DirectX::XMVECTOR) * 2 ,	D3D11_INPUT_PER_VERTEX_DATA, 0 },//法線
		{ "TANGENT",	0, DXGI_FORMAT_R32G32B32_FLOAT, 0, sizeof(DirectX::XMVECTOR) * 3 ,	D3D11_INPUT_PER_VERTEX_DATA, 0 },//従法線
	};
	hr = pDevice->CreateInputLayout(layout, ARRAYSIZE(layout), pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_NORMALMAP].pVertexLayout);
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_NORMALMAP].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ピクセルシェーダの作成に失敗", NULL, MB_OK);
	...
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
	rdc.MultisampleEnable = false;
	hr = pDevice->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TYPE::SHADER_NORMALMAP].pRasterizerState);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"ラスタライザステートの作成に失敗", NULL, MB_OK);
		return hr;
